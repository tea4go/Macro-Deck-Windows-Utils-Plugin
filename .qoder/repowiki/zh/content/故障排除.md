...
## 更新摘要
**变更内容**
- 新增插件更新与维护章节，详细介绍PowerShell自动更新脚本功能
- 添加update-plugin.ps1脚本的使用方法、参数说明、备份机制、错误处理等内容
- 更新故障排除指南以包含插件更新相关的常见问题和解决方案
...
## 简介
本指南面向使用 Macro Deck 2 的"Windows Utils"插件用户与维护者，聚焦于常见问题的系统化排查与解决，涵盖安装部署、配置错误、权限与兼容性问题、启动与窗口管理异常、热键冲突与系统控制异常等。文档同时提供调试方法、日志分析要点、错误代码解释以及性能优化与安全建议，帮助快速定位并解决问题。**新增**插件更新与维护章节，提供PowerShell自动更新脚本的使用指南和最佳实践。

## 项目结构
该插件为 Macro Deck 2 的扩展，通过注册一组动作（Action）实现对 Windows 系统的常用控制：命令行执行、应用启动/切换、音量控制、热键发送、通知、电源选项、窗口激活等。构建脚本与自动部署流程由项目文件与批处理脚本共同驱动。**新增**PowerShell自动更新脚本用于简化插件的版本升级和维护。
...
H["update-plugin.ps1<br/>PowerShell自动更新脚本"] --> E
```
...
- [update-plugin.ps1:1-185](file://update-plugin.ps1#L1-L185)
...
- **新增**自动更新系统：提供PowerShell脚本实现插件的自动化更新与维护。
...
## 架构总览
下图展示插件在 Macro Deck 2 中的运行时交互：按钮触发动作，动作读取配置并通过输入模拟或系统调用执行；部分动作依赖工具与服务完成复杂逻辑。**新增**更新流程集成到整体架构中。
...
- **新增**更新脚本依赖 PowerShell 5.0+ 和 GitHub API 访问权限。
...
U["update-plugin.ps1"] --> GH["GitHub API"]
U --> PS["PowerShell 5.0+"]
U --> MD["Macro Deck 2"]
```
...
- **新增**更新脚本：网络请求和文件操作可能影响系统性能，建议在系统空闲时执行更新。
...
### PowerShell自动更新脚本概述
PowerShell自动更新脚本（update-plugin.ps1）提供了便捷的插件版本管理和维护功能。该脚本可以自动检测最新版本、下载更新包、备份现有文件、执行更新并可选择自动重启Macro Deck 2。

### 脚本功能特性
- **版本检测**：自动查询GitHub Releases获取最新版本信息
- **智能备份**：可选择跳过备份或自动创建完整文件备份
- **安全更新**：停止Macro Deck 2进程后再执行文件覆盖
- **自动重启**：可选择更新后自动启动Macro Deck 2
- **详细日志**：提供完整的执行过程日志和状态反馈
...
# 不自动重启的更新
powershell -ExecutionPolicy Bypass -File .\update-plugin.ps1 -AutoRestart:$false
...
#### 参数说明
- `-AutoRestart`：更新完成后自动重启Macro Deck 2
- `-SkipBackup`：跳过备份步骤，直接覆盖文件

### 更新流程详解

#### 1. 环境检查
脚本首先检查插件目录是否存在，确保插件已正确安装。

#### 2. 版本比较
读取当前安装版本与GitHub最新版本进行比较，只有当版本不同时才执行更新。

#### 3. 下载准备
查找并下载适用于当前系统的Windows-Utils压缩包。

#### 4. 安全更新
- 停止正在运行的Macro Deck 2进程
- 创建备份目录（除非使用SkipBackup参数）
- 下载并解压更新包
- 覆盖插件文件

#### 5. 验证与重启
验证安装版本并根据参数决定是否自动重启Macro Deck 2。

### 常见更新问题及解决方案

#### 1. GitHub API访问失败
**症状**：脚本无法连接到GitHub API获取最新版本信息
**解决方案**：
- 检查网络连接和防火墙设置
- 确认能够访问api.github.com
- 如需代理，配置系统代理设置
...
#### 3. 权限不足
**症状**：无法停止Macro Deck 2进程或覆盖文件
**解决方案**：
- 以管理员权限运行PowerShell
- 确认当前用户对插件目录有写入权限
- 关闭所有Macro Deck 2实例后再运行脚本

#### 4. 备份失败
**症状**：备份创建失败，脚本继续执行但无备份
**解决方案**：
- 检查临时目录空间是否充足
- 确认TEMP环境变量指向有效的磁盘空间
- 手动创建备份目录并确保有写入权限
...
**解决方案**：
- 使用-bypass参数绕过执行策略
- 确认系统满足PowerShell 5.0+要求
- 重新下载脚本文件

#### 2. 更新后插件不可用
**检查清单**：
- 插件目录权限
- ExtensionManifest.json完整性
- Macro Deck 2版本兼容性
...
#### 3. 版本信息显示异常
**检查清单**：
- ExtensionManifest.json格式
- 版本号格式规范
- GitHub Releases标签命名

**解决方案**：
- 手动编辑ExtensionManifest.json
- 确认版本号符合语义化版本规范
- 检查GitHub Releases发布状态

**章节来源**
- [update-plugin.ps1:1-185](file://update-plugin.ps1#L1-L185)

## 结论
通过系统化的问题分类与针对性的排查步骤，大多数与"Windows Utils"插件相关的问题均可快速定位与修复。**新增的PowerShell自动更新脚本进一步简化了插件的维护工作**，但仍需关注权限、配置与兼容性三大维度，并结合日志与调试工具进行深入分析。建议在日常使用中定期检查更新，保持插件版本的最新状态。
...
- **新增**更新脚本：[update-plugin.ps1:1-185](file://update-plugin.ps1#L1-L185)
...
### B. 更新脚本参数参考
- **AutoRestart**：布尔值，控制更新后是否自动重启Macro Deck 2
- **SkipBackup**：布尔值，控制是否跳过备份步骤
- **执行策略**：需要Bypass执行策略以运行脚本

**章节来源**
- [update-plugin.ps1:7-19](file://update-plugin.ps1#L7-L19)
- [update-plugin.ps1:13](file://update-plugin.ps1#L13)