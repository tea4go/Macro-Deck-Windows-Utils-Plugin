...
## 更新摘要
**所做更改**
- 更新 GitHub Actions 自动化发布工作流章节，反映从 dotnet build 迁移到 dotnet publish 的变更
- 新增依赖文件(.deps.json)验证机制的说明
- 更新发布流程，包含更新脚本的自动打包和分发
- 更新版本管理章节，反映版本号提升到 1.6.1
- 增强 PowerShell 更新脚本的使用指南，新增 Force 参数支持
...
**最新更新**：项目现已集成完整的 GitHub Actions 自动化发布工作流，支持基于版本标签的自动发布、打包和上传功能，版本管理增强至 1.6.1。发布流程现在自动包含并分发 update-plugin.ps1 PowerShell 更新脚本，简化了插件的安装和更新过程。工作流已从传统的 dotnet build 迁移到 dotnet publish，确保发布的输出包含正确的依赖文件(.deps.json)。
...
Q["发布工件<br/>DLL + 清单 + 更新脚本 + 依赖文件"] --> R["GitHub Release"]
```
...
- **新增** PowerShell 更新脚本
  - 自动化下载和安装最新版本插件，支持备份、重启、强制更新和参数控制。
  - 参考路径：[update-plugin.ps1:1-192](file://update-plugin.ps1#L1-192)
...
- [.github/workflows/build.yml:45-74](file://.github/workflows/build.yml#L45-L74)
...
- 功能概述
  ...
  - 支持自动备份、智能版本检测、可选的自动重启和强制更新
  - 提供详细的进度反馈和错误处理
- 核心特性
  - 版本比较：自动查询 GitHub Releases 并与当前版本对比
  - 智能备份：更新前自动创建备份目录
  - 安全停止：自动检测并停止正在运行的 Macro Deck 实例
  - 参数控制：支持 `-AutoRestart`、`-SkipBackup` 和 `-Force` 参数
- 使用场景
  - 用户无需手动下载和解压 ZIP 文件
  - 支持无人值守的插件更新流程
  - 提供完整的错误恢复机制
...
- 版本与清单
  ...
  - **版本管理增强**：当前版本为 1.6.1，包含 AssemblyVersion、FileVersion 和 Version 属性。
- CI 工作流
  - 在 master 分支推送与 PR 时触发，使用 .NET 10 进行还原与发布，验证发布输出包含 DLL 和 .deps.json 文件，上传插件 DLL 产物。
...
A --> E["版本信息<br/>1.6.1"]
F[".github/workflows/build.yml"] --> G["还原/发布/验证输出"]
F --> H["自动化发布<br/>基于标签 v*"]
F --> I["更新脚本打包<br/>update-plugin.ps1"]
J["ExtensionManifest.json"] --> K["插件元数据<br/>1.6.1"]
L["GitHub Actions"] --> F
M["PowerShell 更新脚本"] --> I
N["发布工件<br/>DLL + 清单 + 更新脚本 + 依赖文件"] --> O["GitHub Release"]
```
...
- **新增** 更新脚本性能
  ...
  - **新增** 强制更新模式支持，允许重新安装当前版本以修复损坏的安装
...
- **新增** 更新脚本问题排查
  ...
  - 查看详细的错误日志输出以诊断具体问题
  - **新增** 使用 `-Force` 参数强制重新安装当前版本以修复损坏的安装
...
**更新**：项目现已集成完整的自动化发布工作流，支持基于版本标签的自动发布流程，并自动包含 update-plugin.ps1 更新脚本。工作流已从传统的 dotnet build 迁移到 dotnet publish，确保发布的输出包含正确的依赖文件(.deps.json)。
...
#### 发布流程详解
1. **构建阶段**
   - 使用 .NET 10 SDK 进行还原和发布，使用 `dotnet publish` 命令生成发布输出
   - 验证发布输出包含 Windows Utils.dll 和 Windows Utils.deps.json 文件

2. **打包阶段**（仅在标签触发时）
   - 创建发布目录结构 `release/SuchByte.WindowsUtils`
   - 复制发布输出的所有文件（包括 .deps.json）和扩展清单文件
...
```mermaid
flowchart TD
A["推送标签 v1.6.1"] --> B["GitHub Actions 触发"]
B --> C["构建 .NET 解决方案"]
C --> D["dotnet publish 生成发布输出"]
D --> E["验证包含 DLL 和 .deps.json"]
E --> F{"检查是否为标签"}
F --> |是| G["创建发布目录"]
G --> H["复制发布输出文件"]
...
K --> L["上传 DLL + ZIP + 更新脚本 + 依赖文件"]
F --> |否| M["仅构建测试"]
```

**图表来源**
- [.github/workflows/build.yml:30-74](file://.github/workflows/build.yml#L30-L74)
...
- 测试要求
  ...
  - **新增** 更新脚本测试：验证 PowerShell 脚本的执行权限、网络连接、文件操作功能和强制更新模式。
  - 参考路径：[GUI/CommandSelector.cs:46-79](file://GUI/CommandSelector.cs#L46-L79)，[Actions/CommandlineAction.cs:22-58](file://Actions/CommandlineAction.cs#L22-L58)，[update-plugin.ps1:1-192](file://update-plugin.ps1#L1-192)
...
- 参考路径：[README.md:20-28](file://README.md#L20-L28)，[Resources/Languages/README.md:15-29](file://Resources/Languages/README.md#L15-L29)，[update-plugin.ps1:1-192](file://update-plugin.ps1#L1-192)
...
- 本地构建与调试
  - 使用解决方案进行发布；启用 PostBuild 事件可自动复制 DLL 到 Macro Deck 插件目录。
...
- 验证步骤
  ...
  - **新增** 验证更新脚本：测试 PowerShell 脚本的执行、备份功能、版本检测和强制更新模式。
...
**更新**：项目版本管理已增强至 1.6.1，包含多个版本属性和增强功能，并集成了自动化更新脚本。

#### 版本属性
- **AssemblyVersion**: 1.6.1.0（程序集版本）
- **FileVersion**: 1.6.1.0（文件版本）
- **Version**: 1.6.1（显示版本）
