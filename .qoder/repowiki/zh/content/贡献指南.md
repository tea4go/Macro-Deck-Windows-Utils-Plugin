...
## 更新摘要
**所做更改**
- 新增 GitHub Actions 自动化发布工作流章节，详细说明基于标签的自动发布流程
- 更新版本管理章节，反映 1.6.0 版本号和版本管理增强
- 新增自动化构建与打包流程说明
- 更新 CI/CD 流程，包含发布制品和版本标签处理
...
**最新更新**：项目现已集成完整的 GitHub Actions 自动化发布工作流，支持基于版本标签的自动发布、打包和上传功能，版本管理增强至 1.6.0。

## 项目结构
该项目采用基于功能域的组织方式：核心插件入口在根目录，功能动作位于 Actions，图形界面控件位于 GUI，模型与视图模型位于 Models/ViewModels，工具类位于 Utils，服务层位于 Services，国际化资源位于 Resources/Languages，并通过 Language 子模块进行统一加载与管理。
...
## 架构总览
下图展示了插件从启用到执行动作的关键交互流程，包括语言资源加载、动作注册、配置控件与动作触发之间的关系。
...
F --> H["自动化发布<br/>基于标签 v*"]
I["ExtensionManifest.json"] --> J["插件元数据<br/>1.6.0"]
K["GitHub Actions"] --> F
```
...
- [.github/workflows/build.yml:14-65](file://.github/workflows/build.yml#L14-L65)
...
## 结论
本指南提供了从开发环境搭建、代码贡献、翻译贡献到 CI 流程与故障排查的完整路径。随着 GitHub Actions 自动化发布工作流的引入和版本管理增强，项目现在具备了更完善的持续集成和发布能力，显著提升了贡献效率与质量，确保新增功能与翻译内容与现有架构保持一致。
...
**更新**：项目现已集成完整的自动化发布工作流，支持基于版本标签的自动发布流程。
...
**图表来源**
- [.github/workflows/build.yml:48-65](file://.github/workflows/build.yml#L48-L65)
...
- 测试要求
  ...
  - 参考路径：[GUI/CommandSelector.cs:46-79](file://GUI/CommandSelector.cs#L46-L79)，[Actions/CommandlineAction.cs:22-58](file://Actions/CommandlineAction.cs#L22-L58)
...
- 参考路径：[README.md:20-28](file://README.md#L20-L28)，[Resources/Languages/README.md:15-29](file://Resources/Languages/README.md#L15-L29)
...
- 验证步骤
  ...
  - 参考路径：[Main.cs:28-58](file://Main.cs#L28-L58)，[Actions/CommandlineAction.cs:22-58](file://Actions/CommandlineAction.cs#L22-L58)
...
**更新**：项目版本管理已增强至 1.6.0，包含多个版本属性和增强功能。
