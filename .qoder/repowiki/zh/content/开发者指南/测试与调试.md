...
**更新** 本次更新重点反映了插件在开发工具链方面的重大改进，包括新增PowerShell自动化脚本(run_win.ps1)、端到端测试脚本(test-editplus.ps1)、插件更新脚本(update-plugin.ps1)、批处理简化脚本(run_win.bat)以及GitHub Actions持续集成配置，显著提升了开发者体验和测试效率。特别值得一提的是，test-editplus.ps1已完全中文化，所有注释、帮助信息和错误消息均提供中文支持，同时新增的run_win.bat批处理文件为Windows用户提供了更简便的执行方式。

## 项目结构
该插件采用"按功能域分层"的组织方式：主入口负责插件生命周期与动作注册；Actions 目录承载各类用户触发的动作；Services 提供系统级能力封装；Utils 提供底层系统调用与工具函数；GUI 与 ViewModels 负责配置界面与视图模型；语言资源与清单文件支撑国际化与插件元数据；新增的PowerShell脚本和批处理文件提供完整的开发自动化工具链。
...
L["run_win.bat<br/>批处理简化脚本"] --> K
M["test-editplus.ps1<br/>端到端测试脚本(已中文化)"] --> A
N["update-plugin.ps1<br/>插件更新脚本"] --> A
O[".github/workflows/build.yml<br/>CI/CD配置"] --> A
P["autorun.bat<br/>快速开发脚本"] --> A
```
...
- [test-editplus.ps1:1-293](file://test-editplus.ps1#L1-L293)
...
- 开发自动化工具链：包括PowerShell脚本(run_win.ps1)提供一键构建、部署和测试功能，批处理脚本(run_win.bat)为Windows用户提供简化的执行方式，端到端测试脚本(test-editplus.ps1)验证完整功能链路，插件更新脚本(update-plugin.ps1)支持从GitHub发布版本自动更新，所有脚本均提供中文支持。
...
participant BAT as "批处理脚本(run_win.bat)"
participant PS as "PowerShell脚本(run_win.ps1)"
...
Dev->>BAT : 双击运行run_win.bat
BAT->>PS : 调用powershell -ExecutionPolicy Bypass -File run_win.ps1
PS->>PS : 检测.NET SDK
PS->>PS : 构建并发布插件
PS->>UI : 替换安装插件
...
### 批处理简化脚本（run_win.bat）
职责：为Windows用户提供简化的执行方式，通过批处理文件调用PowerShell脚本。
关键特性：
- 自动调用PowerShell并绕过执行策略限制
- 支持传递所有命令行参数给底层PowerShell脚本
- 提供最简化的Windows执行流程，降低学习成本

```mermaid
flowchart TD
Start(["双击run_win.bat"]) --> CallPS["powershell -ExecutionPolicy Bypass -File run_win.ps1 %*"]
CallPS --> PassArgs["传递所有命令行参数"]
PassArgs --> Done(["完成"])
```
...
**图表来源**
- [test-editplus.ps1:64-293](file://test-editplus.ps1#L64-L293)
...
- **新增** 批处理简化依赖：Windows批处理环境，支持PowerShell执行策略绕过。
...
- **新增** 开发工具链性能：
  ...
  - **新增** 批处理脚本性能：run_win.bat作为轻量级包装器，几乎不影响执行性能，但提供了更便捷的Windows用户体验。
...
## 结论
本指南基于仓库现有实现总结了测试与调试的关键路径：以动作为中心的触发链路、服务层的系统交互、工具层的底层调用与日志调试。新增的开发自动化工具链显著提升了开发效率和测试质量，包括PowerShell脚本的完整开发流程自动化、端到端测试脚本的全面功能验证、插件更新脚本的版本管理能力、批处理脚本的Windows用户友好体验，以及GitHub Actions的持续集成配置。特别值得强调的是，test-editplus.ps1已完全中文化，为中文用户提供了更好的本地化体验，而run_win.bat批处理文件进一步简化了Windows用户的执行流程。

建议在开发周期中优先完善单元测试（配置解析、异常分支）、集成测试（进程/窗口/P/Invoke 场景）与端到端测试（真实宿主环境），并结合增强的调试宏命令与日志输出建立闭环问题定位流程。新的工具链为开发者提供了从代码编写到功能验证的一站式解决方案，其中批处理脚本和中文化脚本的加入使得整个开发工具链更加完善和用户友好。
...
- **新增** 端到端测试
  ...
  - **新增** 中文化验证：确保test-editplus.ps1的所有输出均为中文，验证本地化效果。
  - **新增** 执行方式验证：通过run_win.bat验证批处理脚本的正确性和易用性。
...
- **新增** 开发工具链调试
  ...
  - **新增** 批处理脚本调试：通过run_win.bat验证PowerShell执行策略设置，检查脚本路径和权限。
    - 参考：[run_win.bat:1-3](file://run_win.bat#L1-L3)
  - 端到端测试调试：通过test-editplus.ps1的详细日志输出定位问题，使用SkipUpdate参数跳过更新步骤。
    - 参考：[test-editplus.ps1:1-293](file://test-editplus.ps1#L1-L293)
...
- **新增** 批处理简化执行
  - run_win.bat提供Windows用户的简化解析器执行方式，通过PowerShell绕过执行策略限制。
    - 参考：[run_win.bat:1-3](file://run_win.bat#L1-L3)
- **新增** 端到端自动化测试
  - test-editplus.ps1提供完整的端到端测试流程，支持跳过更新和自定义参数，已完全中文化。
...
- **新增** 案例六：批处理脚本无法执行PowerShell
  - 步骤：检查PowerShell执行策略设置为Bypass或RemoteSigned；确认run_win.ps1路径正确；验证脚本权限。
  - 参考：[run_win.bat:1-3](file://run_win.bat#L1-L3)
- **新增** 案例七：端到端测试连接WebSocket失败
  - 步骤：确认Macro Deck WebSocket端口配置；检查客户端ID是否已在devices.json中配对；验证防火墙设置；检查test-editplus.ps1的中文输出是否正常。
  - 参考：[test-editplus.ps1:107-134](file://test-editplus.ps1#L107-L134)
- **新增** 案例八：插件更新脚本无法访问GitHub
