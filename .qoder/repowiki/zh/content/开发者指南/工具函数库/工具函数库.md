...
- WindowActivator：基于标题匹配的窗口激活与前置
- ApplicationLauncher：应用程序启动、前台/后台切换、终止与运行状态检测
...
- [ApplicationLauncher.cs:1-165](file://Services/ApplicationLauncher.cs#L1-L165)
...
- [WindowSwitchAction.cs:1-47](file://Actions/WindowSwitchAction.cs#L1-L47)
...
## 核心组件
- WindowActivator：提供按标题模式匹配的窗口查找与激活能力，支持部分匹配、全等匹配、前缀/后缀匹配以及正则表达式匹配，并对任务栏可见性进行过滤。
- ApplicationLauncher：封装应用启动、前台/后台切换、终止与运行状态检测；内部使用快捷方式解析以定位真实可执行路径。
...
- 关键 API
  ...
  - BringToForeground(path)：置前窗口（含回退逻辑）
  - GetProcessByPath(path) -> Process：根据路径获取进程（解析 .lnk）
  - GetProcessFileName(pid) -> string：通过句柄查询进程可执行路径
- 调用序列（启动/前台切换）
...
## 结论
本工具函数库围绕“窗口管理、应用控制、图标处理、图像缩放与快捷方式解析”提供了稳定、可组合的工具集。通过与插件动作的紧密协作，能够满足自动化桌面操作与图标管理的常见需求。建议在生产环境中遵循性能与健壮性原则，合理缓存与短路处理，提升整体响应速度与用户体验。

## 附录
- 最佳使用模式
  - 窗口切换：优先使用前缀/后缀匹配，避免正则开销；若需跨进程置前，使用 ForceActivateWindow
  - 应用启动：先解析 .lnk，再根据运行状态决定启动或置前；管理员权限需谨慎使用
