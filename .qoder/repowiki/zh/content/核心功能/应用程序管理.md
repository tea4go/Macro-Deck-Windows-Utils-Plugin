...
graph TB
subgraph "动作层"
SA["StartApplicationAction<br/>触发器与按钮状态同步<br/>增强错误处理与日志记录"]
end
subgraph "视图层"
...
subgraph "服务层"
L["ApplicationLauncher<br/>进程管理与窗口操作<br/>集成 WindowActivator"]
WA["WindowActivator<br/>高级窗口标题匹配与激活"]
...
**章节来源**
- [Windows Utils.csproj:1-74](file://Windows Utils.csproj#L1-L74)
- [Main.cs:28-61](file://Main.cs#L28-L61)

## 核心组件
本节概述关键组件及其职责：
- StartApplicationAction：作为 Macro Deck 动作，负责根据配置触发应用启动、停止或窗口切换。**新增**：增强了错误处理和日志记录功能，提供更好的用户体验。
- ApplicationLauncher：封装进程启动、终止、前台/后台切换与进程查询逻辑。**新增**：集成了 WindowActivator，提供更强大的窗口激活能力。
- StartApplicationActionConfigModel/ViewModel/View：提供配置项的序列化、绑定与 UI 输入处理。
- WindowActivator：提供基于标题模式的窗口激活能力（用于更广泛的窗口管理场景）。**增强**：支持多种匹配模式和正则表达式，提供更精确的窗口控制。
...
- [WindowsShortcut.cs:5-66](file://Utils/WindowsShortcut.cs#L5-L66)
- [PluginLanguageManager.cs:8-51](file://Language/PluginLanguageManager.cs#L8-L51)
- [Main.cs:16-63](file://Main.cs#L16-L63)

## 架构总览
下图展示了从用户触发到进程管理与窗口操作的整体流程，**更新**：集成了增强的错误处理和日志记录系统。

```mermaid
sequenceDiagram
participant U as "用户"
participant AB as "StartApplicationAction<br/>增强错误处理"
participant CFG as "配置模型/视图模型"
participant LA as "ApplicationLauncher<br/>集成 WindowActivator"
...
- 窗口操作
  ...
  - **增强**：当 MainWindowHandle 为 0（最小化）时，使用 WindowActivator.ActivateWindowByProcessId 激活窗口。
...
```mermaid
graph LR
SA["StartApplicationAction<br/>增强错误处理"] --> AL["ApplicationLauncher<br/>集成 WindowActivator"]
SA --> CFG["配置模型/视图模型/视图"]
...
- 窗口无法激活
  ...
  - **新增**：如果应用最小化，使用 WindowActivator.ActivateWindowByProcessId。
...
## 结论
本应用程序管理子系统通过清晰的动作-服务-配置分层设计，提供了稳定可靠的 Windows 应用启动与窗口管理能力。**最新增强**包括：
- 简洁直观的配置界面与序列化机制。
- 基于路径与快捷方式的目标解析，增强兼容性。
- 完备的进程生命周期管理与窗口状态控制。
- **新增**：集成 WindowActivator 提供更强大的窗口激活能力。
- **新增**：增强的错误处理和日志记录系统。
- 可扩展的多语言支持与日志体系。
...
- **新增**：如果应用最小化，系统会自动使用 WindowActivator 激活窗口。
