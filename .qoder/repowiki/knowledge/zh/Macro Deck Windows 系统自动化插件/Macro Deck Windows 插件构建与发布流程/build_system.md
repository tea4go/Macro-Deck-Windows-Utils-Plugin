...
## 2. 关键配置文件
- **`.github/workflows/build.yml`**: 定义了 CI 流水线。在 `windows-latest` 环境下运行，执行 `dotnet restore` 和 `dotnet build`，并将生成的 `Windows Utils.dll` 作为 Artifact 上传。
- **`Windows Utils.csproj`**: 项目核心配置。指定了目标框架、版本号（当前为 1.5.1）、平台（x64）以及特殊的引用逻辑。
...
- **清单版本**: `ExtensionManifest.json` 中的版本需手动同步，目前存在轻微不一致（csproj 为 1.5.1，manifest 为 1.5.0）。
